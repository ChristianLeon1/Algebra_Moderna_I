...
\begin{tikzpicture}[
    ...
    % Conjuntos G (Elipses)
    \draw[thick, fill=blue!5] (0,0) ellipse (1.5cm and 2.5cm);
    \node at (0, 2.8) {\Large $G$};

    \draw[thick, fill=blue!5] (6.5,0) ellipse (1.5cm and 2.5cm);
    \node at (6.5, 2.8) {\Large $G$};

    % Elementos
    \node (g) at (0, 0.5) {$\bullet \ g$};
    \node (aga) at (6.5, 0.5) {$\bullet \ aga^{-1}$};

    % Flecha del mapeo f_a
    \draw[->, thick, blue] (g) to[bend left=20] node[midway, above] {$f_a$} (aga);
    \node[blue, below] at (3.2, 0.8) {Conjugación por $a$};

    % Identidad (opcional para mostrar que e va a e)
    \node (e1) at (0, -1.5) {$\bullet \ e$};
    \node (e2) at (6.5, -1.5) {$\bullet \ e$};
    \draw[->, dashed] (e1) -- (e2);
...
\begin{proof}
    ...
    \end{equation*}
Podemos visualizar la acción del automorfismo inducido a través del siguiente diagrama:
    \begin{center}
\begin{tikzpicture}[
    node distance=2.5cm,
    % Ajustamos el estilo de flecha para usar arrows.meta (ya que lo tienes cargado)
    >={Stealth[scale=1.2]}, 
    every node/.style={minimum width=1.2cm, minimum height=1.8cm},
    arrow/.style={->, thick},
    curve/.style={->, thick, bend right=25}
]

% Nodos
\node (G0) {$G_i$};
\node (G1) [right=of G0] {$G$};
\node (G2) [right=of G1] {$G$};
\node (G3) [right=of G2] {$G_{i+1}$};

% Flechas superiores
\draw[arrow] (G0) -- node[above] {$\psi_i$} (G1);
\draw[arrow] (G1) -- node[above] {$f$} (G2);
\draw[arrow] (G2) -- node[above] {$\psi$} (G3);

% Flecha curva inferior
\draw[curve] (G0.south) to node[below] {$\psi_{i+1}$} (G3.south);
...
\begin{proof}
    ...
    \begin{itemize}
        \item Inyectiva: Si $\Psi(f) = (Id_{H_1}, Id_{H_2})$, entonces $f(h, e_2) = (h, e_2)$ y $f(e_1, k) = (e_1, k)$.
        ...
        \item Sobreyectiva: Dado $(\alpha, \beta) \in \text{Aut}(H_1) \times \text{Aut}(H_2)$, definimos $f: H_1 \times H_2 \to H_1 \times H_2$ como $f(h, k) = (\alpha(h), \beta(k))$.
