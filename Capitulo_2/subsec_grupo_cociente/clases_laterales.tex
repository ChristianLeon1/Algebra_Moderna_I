...
\begin{prop}
    Sea $(G, \circ, e)$ un grupo y sea $H \le G$. Entonces, los conjuntos de clases laterales izquierdas $\mathcal{L}$ y derechas $\mathcal{R}$ forman, cada uno, una partición de $G$.
\end{prop}

\begin{proof}
    Para demostrar esto, basta probar que la colección $\mathcal{R}$ induce una relación de equivalencia en $G$. Definimos la relación $\sim$ de la siguiente manera: dados $a, b \in G$, diremos que $a \sim b$ si y solo si existe un elemento $g \in G$ tal que ambos pertenecen a la misma clase lateral derecha generada por $g$. Es decir:
    \begin{equation*}
        a \sim b \iff \exists g \in G \text{ tal que } a, b \in Hg.
    \end{equation*}

   A continuación, verificamos las propiedades requeridas:

    \begin{enumerate}[label=\roman*), font=\normalfont]
        \item \textsc{Reflexividad:} Para todo $a \in G$, se cumple que $a \sim a$.
        Esto es evidente dado que $e \in H$, lo que implica que $a = ea \in Ha$. Por lo tanto, tomando $g = a$, tenemos que $a, a \in Ha$, lo cual satisface la definición.

        \item \textsc{Simetría:} Supongamos que $a \sim b$.
        Por definición, existe $g \in G$ tal que $a, b \in Hg$. El orden en el que enunciamos la pertenencia al conjunto no altera el hecho lógico; es decir, $b, a \in Hg$. En consecuencia, $b \sim a$.

        \item \textsc{Transitividad:} Supongamos que $a \sim b$ y $b \sim c$.
        Debemos demostrar que $a \sim c$. Por la hipótesis, sabemos que existen $g_1, g_2 \in G$ tales que:
        \begin{equation*}
            a, b \in Hg_1 \quad \text{y} \quad b, c \in Hg_2.
        \end{equation*}
        Esto implica que existen elementos $h_1, h_2, h_3, h_4 \in H$ que satisfacen las siguientes igualdades:
        \begin{align*}
            a &= h_1 g_1, & b &= h_2 g_1, \\
            b &= h_3 g_2, & c &= h_4 g_2.
        \end{align*}
        Observamos que tenemos dos expresiones para $b$. Igualándolas obtenemos una relación entre los generadores $g_1$ y $g_2$:
        \begin{align*}
            h_2 g_1 &= h_3 g_2 \\
            g_1 &= h_2^{-1} h_3 g_2.
        \end{align*}
        Ahora sustituimos esta expresión de $g_1$ en la ecuación original de $a$:
        \begin{align*}
            a &= h_1 (h_2^{-1} h_3 g_2) \\
              &= (h_1 h_2^{-1} h_3) g_2.
        \end{align*}
        Dado que $H$ es un subgrupo, el producto y la inversa de elementos de $H$ permanecen en $H$. Sea $h' = h_1 h_2^{-1} h_3 \in H$. Entonces:
        \begin{equation*}
            a = h' g_2 \implies a \in Hg_2.
        \end{equation*}
        Como ya sabíamos que $c \in Hg_2$, concluimos que $a, c \in Hg_2$, lo que demuestra que $a \sim c$.
    \end{enumerate}

    Al ser $\sim$ una relación de equivalencia, sus clases de equivalencia forman una partición del conjunto $G$. Dado que la clase de equivalencia de un elemento $a$ bajo esta relación es precisamente la clase lateral $Ha$, concluimos que $\mathcal{R}$ forma una partición de $G$.

    Análogamente, se demuestra que las clases laterales izquierdas $\mathcal{L}$ forman una partición de $G$.
\end{proof}
...
\begin{proof}
	Sea $|G|=n$, tome el conjunto $\{g^{i_1}, \dots, g^{i_{n+1}}\}$, con $i_1, \dots, i_{n+1} \in \mathbb{Z}^+$ con $i_1 < i_2 < \dots < i_{n+1}$. Entonces al menos 2 son iguales, es decir $g^{i_j} = g^{i_k}$, para $1 \le j < k \le n+1$.
	Luego:
	\begin{equation*}
...
\import{../images/Capitulo_2/}{reticulo_ejemplo_S3.tex}

\ObservacionBox{Observación}{
	Sea $(G, \circ, e)$ un grupo, ser conjugado es una relación de equivalencia.
}

\begin{proof}
	\begin{itemize}
		\item \textsc{Reflexividad:} $H \sim H$ pues $eHe^{-1} = H \quad \forall H \le G$.

		\item \textsc{Simetría:} Sean $H_1, H_2$, tal que $H_1 \sim H_2$, entonces $H_1 = g H_2 g^{-1}$, luego: $g^{-1}H_1 g = H_2$, así $H_2 \sim H_1$.

		\item \textsc{Transitiva:} Sean $H_1, H_2, H_3 \le G$ tal que $H_1 \sim H_2, H_2 \sim H_3$, entonces existen $g_1, g_2 \in G$ tal que $H_2 = g_1 H_1 g_1^{-1}$, $H_3 = g_2 H_2 g_2^{-1} = g_2 (g_1 H_1 g_1^{-1}) g_2^{-1} = (g_2 g_1) H_1 (g_2 g_1)^{-1}$.
		      Dado que $g_2 g_1 \in G$, $H_1 \sim H_3$.

		      Además note que son conjugados $|H_1| = |H_3|$. Si son conjugados $\exists g \in G$ tal que $H_3 = g H_1 g^{-1}$, sea $\varphi: H_1 \to H_3$, dada por $\varphi(h_1) = g h_1 g^{-1}$, podemos probar que es biyectiva.
	\end{itemize}
\end{proof}

\begin{eje}
	Sea $Q_8 = \{ \pm 1, \pm i, \pm j, \pm k \}$, con $i^2 = j^2 = k^2 = -1$, $ij=k$, $jk=i$, $ki=j$, $-k=ji$, $-i=kj$, $-j=ki$. Calcule los subgrupos normales y calcule los conjugados.
\end{eje}

\import{../images/Capitulo_2/}{reticulo_ejemplo_Q8.tex}
